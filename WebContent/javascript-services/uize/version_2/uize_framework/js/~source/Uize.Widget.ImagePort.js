	...
	builder:function (_superclass) {
		...
		/*** Public Instance-Static Methods ***/
			_classPrototype.getScaledRect = _class.getScaledRect = function (_params) {
				function _getScaleFactor (_sizing) {
					return _sizing == 'fit' ? _fitScaleFactor : (_sizing == 'fill' ? _fillScaleFactor : 0);
				}
				...
				var
					...
					_scaleFactorLowerBound = _getScaleFactor (_getDefaultedProperty ('sizingLowerBound')),
					_scaleFactorUpperBound = _getScaleFactor (_getDefaultedProperty ('sizingUpperBound')),
			...
			_classPrototype.getSizingAndAlign = _class.getSizingAndAlign = function (_params) {
				function _getScaledVolume (_sizing) {
					return _sizing == 'fit' ? _fitVolume : (_sizing == 'fill' ? _fillVolume : 0);
				}
				function _getDefaultedProperty (_propertyName) {
					return _params [_propertyName] !== _undefined ? _params [_propertyName] : _this.get (_propertyName);
				}
				var
					...
					_rectWidth = _params.rectWidth,
					_rectHeight = _params.rectHeight,
					_rectVolume = _rectWidth * _rectHeight,
					_fillScaleFactorWidth = _portWidth / _rectWidth,
					_fillScaleFactorHeight = _portHeight / _rectHeight,
					_fitScaleFactor = Math.min (_fillScaleFactorWidth,_fillScaleFactorHeight),
					_fillScaleFactor = Math.max (_fillScaleFactorWidth,_fillScaleFactorHeight),
					_fitVolume = _fitScaleFactor * _fitScaleFactor * _rectVolume,
					_fillVolume = _fillScaleFactor * _fillScaleFactor * _rectVolume,
					_scaledVolumeLowerBound = _getScaledVolume (_getDefaultedProperty ('sizingLowerBound')),
					_scaledVolumeUpperBound = _getScaledVolume (_getDefaultedProperty ('sizingUpperBound'))
				;
				return {
					sizingValue:
						Math.sqrt (_rectVolume - _scaledVolumeLowerBound) /
						Math.sqrt (_scaledVolumeUpperBound - _scaledVolumeLowerBound)
					,
		...
		/*** Register Properties ***/
			...
			_class.registerProperties ({
				_alignApplicableX:'alignApplicableX',
				_alignApplicableY:'alignApplicableY',
				_alignX:{
					name:'alignX',
