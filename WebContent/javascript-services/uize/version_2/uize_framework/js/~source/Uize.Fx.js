	...
	Introduction
		...
		*DEVELOPERS:* `Chris van Rensburg`
		...
		Extensions For CSS3 Support
			...
			An example is the =Uize.Fx.xTextShadow= module, which provides support for fading values for the =text-shadow= CSS3 style property.
*/
	...
	builder:function () {
				...
				/*?
					Static Methods
						Uize.Fx.defineStylePropertiesProfile
							...
							As an example, the =Uize.Fx.xTextShadow= module uses the =Uize.Fx.defineStylePropertiesProfile= method to add support for the CSS3 =text-shadow= style property. Once this module has been loaded, the =Uize.Widget.HoverFader= widget class, which uses the =Uize.Fx.fadeStyle= method in its implementation, will be able to support fading of text shadows.
		...
		/*** Initialization ***/
			...
			/*** dimension style properties ***/
				_defineStylePropertiesProfile ({
					test:/(fontSize|left|top|right|bottom|width|height|padding|margin|spacing|textIndent)/i,
					decoder:function (_value) {return typeof _value == 'number' ? _value : parseInt (_value) || 0},
