...
Uize.module ({
	...
	builder:function () {
		...
		/*** Utility Functions ***/
			var _regExpEscapedWhitespaceChars = {
				'\a':'a',  // bell
				'\e':'e',  // escape
				'\f':'f',  // form feed
				'\r':'r',  // CR (carriage return)
				'\n':'n',  // LF (line feed / new line)
				'\t':'t'   // tab
			};
			function _escapeRegExpLiteral (_sourceStr) {
				return _sourceStr.replace (
					/[\a\e\f\r\n\t\^\$\|\.\?\*\+\,\:\[\]\(\)\{\}\\]/g,
					function (_match) {return '\\' + (_regExpEscapedWhitespaceChars [_match] || _match)}
				);
			}
			...
			function _globalLiteralRegExp (_replaceStr) {
				return new RegExp (_escapeRegExpLiteral (_replaceStr),'g');
			}
