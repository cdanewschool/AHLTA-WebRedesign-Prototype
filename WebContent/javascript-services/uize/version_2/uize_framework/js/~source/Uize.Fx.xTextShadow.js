...
/*?
	Introduction
		The =Uize.Fx.xTextShadow= extension module implements support for animating the CSS3 =text-shadow= property, enabling a wide variety of stunning effects.

		*DEVELOPERS:* `Chris van Rensburg`

		The =Uize.Fx.xTextShadow= module is an extension module under the =Uize.Fx= namespace.

		In a Nutshell
			Using this module, color, horizontal offset, vertical offset, and blur radius properties for an arbitrary number of text shadows can be animated between one state and another, making a vast array of interesting effects possible.

			The [[http://www.w3.org/TR/css3-text/#text-shadow][text-shadow]] property is defined in the [[http://www.w3.org/TR/css3-text/][CSS Text Level 3]] specification. At the time of this writing, this property is supported only in certain newer browsers, such as Google Chrome 2.x+, Safari 4.x+, Firefox 3.1+, Opera 9.5+, Konqueror, and other browsers based on the recent [[http://webkit.org/][WebKit]] project.
		...
		Why An Extension?
			Because support for text shadow by Web browsers is likely to take some time to become pervasive, support for fading this property is not built directly into the =Uize.Fx= base module, but is instead provided in this separate extension.

			Fading values for the text shadows of DOM nodes can produce some [[../examples/hover-fader-text-shadow-animation.html][spectacular effects]], but such effects can be considered esoteric enough that the weight of the framework should not be increased for the majority of use cases for the sake of supporting text shadow animation. In cases where text shadow animation is desired, it is easy enough to simply load in the =Uize.Fx.xTextShadow= module.

		Whet Your Appetite
			If you are not yet familiar with text shadow and the kinds of effects that are possible when animating shadows, you can whet your appetite by taking a look at the [[../examples/hover-fader-text-shadow-animation.html][Hover Fader Text Shadow Animation]] example.

			Once you are done checking out the example and feel inspired to learn more about the nitty-gritty of animating shadows, then you can return to this reference to get the full scoop.

		A Basic Example
			Before getting into a detailed discussion of text shadow animation, let's take a look at a very basic example...
	...
	Key Features
		...
		Supports an Arbitrary Number of Shadows
			...
			The =Uize.Fx.xTextShadow= module supports fading values for all shadow properties of an arbitrary number of shadows. You can produce some pretty sophisticated animation effects when you involve multiple shadows for the same node, but be aware of `Performance Considerations` when doing this.
		...
		### advanced techniques
			- applying different curves to different parts of a text shadow animation

	Important Considerations
		Not All Browsers Support Text Shadow
			At the time of writing, a number of the leading Web browsers support text shadow, but the overwhelming majority of the installed base of browsers still do not.

			Browsers that provide complete support for text shadow include...
			...
			Internet Explorer
				Neither IE6, IE7, nor IE8 support text shadow, and it is not yet clear whether IE9 will support text shadow.

				We hope that IE9 *does* support text shadow, because text shadow can be very compelling in providing rich visualizations. Hopefully Web developers will strongly express their desire for full CSS3 support, and hopefully Microsoft rise to the challenge.

			Graceful Degradation
				Given the reality that text shadow will take a while to become universally supported in a majority of installed browsers, it is important to employ the principle of graceful degradation when using text shadow.

				Quite often an element that is styled with text shadow can also look satisfactory when text shadow is not present. At the very least, color choices should be made that ensure the text remains legible when text shadow is not supported. Don't rely on the text shadow, in other words, in order for the text to be visible. The best way to achieve gracefuly degradation is to simply test your content in browsers that don't support text shadow and tune text and background colors accordingly to make sure the fallback is acceptable.

		Performance Considerations
			When animating text shadow effects, keep in mind that rendering text shadows for large amounts of text (or for many DOM nodes) can be costly enough to the browser as to reduce the number of animation updates that can be performed.

			Therefore, when animating text shadow for many DOM nodes simultaneously, animation may become jerky. Animation performance is also impacted by the number of shadows being animated per element, and other factors such as the blur radius for shadows. Keep this in mind when designing effects using text shadow animation, and test in a few browsers that support text animation to see if your effects need to be "dialed back" a bit to perform adequately in all.

		Match Structure of Start and End Values
			When animating text shadow between a start value and an end value, it is important that the structure of the start value is the same as that of the end value in the following key ways...

			Use All the Same Units For Start and End Values
				While the units for different properties of a text shadow don't all have to be the same, all spacial properties of all shadows for the start value should have the same unit as the *corresponding* properties of the *corresponding* shadows in the end value.
			...
			Same Number of Shadows
				Both the start value and the end value for the =textShadow= property should contain the same number of shadows.
...
Uize.module ({
	name:'Uize.Fx.xTextShadow',
	required:'Uize.Color',
	builder:function (_package) {
		var _extension = function () {};

		/*** Private Static Methods ***/
			for (var _serializeTextShadowMethodName in {_serializeTextShadow:1});
			_extension._serializeTextShadow = function (_value) {
				var _shadows = [];
				for (var _shadowNo = -1, _valueLength = _value.length, _shadow; ++_shadowNo < _valueLength;)
					_shadows.push (
						(_shadow = _value [_shadowNo]).offsetX + _shadow.offsetXUnit +
		...
		_package.defineStylePropertiesProfile ({
			...
			decoder:function (_value) {
				if (Uize.isArray (_value)) _value = _value.join (',');
				var
					_result = [],
				...
				;
				while (_shadowPropertiesMatch = _shadowPropertiesRegExp.exec (_value)) {
					var
			...
			encoderExpression:'Uize.Fx.xTextShadow.' + _serializeTextShadowMethodName + '(VALUE)'
		});
