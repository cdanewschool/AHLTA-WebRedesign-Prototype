	...
	builder:function (_superclass) {
		...
		/*** Global Variables ***/
			var
				...
				_browserHasSelectShowThroughIssue =
					_navigator.appName == 'Microsoft Internet Explorer' && _navigator.appVersion.indexOf ('MSIE 6') > -1
			;
		...
		/*** Private Instance Methods ***/
			...
			_classPrototype._updateUiShieldOpacity = function () {
				var _this = this;
				_this.isWired && _this._shown &&
					_this.setNodeOpacity ('shield',_this._currentShieldOpacity)
				;
			};
			...
			_class.registerProperties ({
				...
				_shown:{
					...
					onChange:function () {
						var _this = this;
						if (_this.isWired) {
							function _updateSelectNodeVisibilityForIe6Issue () {
								if (_browserHasSelectShowThroughIssue) {
									/* ISSUES: (in order of priority)
										- if some dialogs are shown, only select nodes contained inside the root node of the topmost dialog should be shown. This is not currently the way the code below is behaving at all. Moreover, there is no global variables that is a list of all dialog instances, so this code can't know the topmost dialog instance. Perhaps _zIndexSlots should map to the instances, but then we need to ensure that references don't linger beyond remove -- tricky.

										- this code doesn't handle cases where there are IFRAME dialogs lower in the stack with select tags, or even iframe in the document - for that matter - with select tags

										- this code assumes it is safe to obliterate the value of the visibility style property, but there might be cases where other logic is using this property (so this solution is non-cooperative)
									*/
									var _rootNode = _this.getNode ();
									_Uize_Node.doForAll (
										document.getElementsByTagName ('select'),
										function (_node) {
											_Uize_Node.show (_node,!_totalShown || _Uize_Node.isOnNodeTree (_node,_rootNode));
										}
									);
								}
							}
							if (_this._shown) {
								...
								_updateSelectNodeVisibilityForIe6Issue ();
