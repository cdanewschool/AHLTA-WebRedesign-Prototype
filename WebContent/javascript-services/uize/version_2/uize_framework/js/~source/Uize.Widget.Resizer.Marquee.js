							...
							_this.wireNode (
								...
								function (_event) {
									...
									_this.setPositionDuringDrag (
										_event.clientX + _documentElement.scrollLeft - _shellCoords.left,
										_event.clientY + _documentElement.scrollTop - _shellCoords.top,
										_this.get ('minWidth'),
										_this.get ('minHeight')
									);
									return _this.children [_handleName].mousedown (_event);
								}
							);
						}
				...
				html:{
					process:function (input) {
						function _getHandleHtml (_handleName) {
							return (
								'<img id="' + input.idPrefix + '-' + _handleName + '" src="' + input.blankGif + '"' + (input.handleCssClass ? (' class="' + input.handleCssClass + '"') : '') + ' style="position:absolute; z-index:1000; display:none;' + (input.handleCssClass ? '' : (' width:7px; height:7px; background:#ccc; border:1px solid #fff;')) + '"/>'
							);
						}
						return (
